...
const ProjectCard = ({ project }: { project: ProjectProps }) => {
    ...
    return (
        <div
            ...
            className={`group relative glass-effect rounded-lg overflow-hidden transition-all duration-500 flex flex-col cursor-pointer ${isOpen ? 'active-card z-10' : ''}`}
        >
            {/* Project Thumbnail */}
            <div
            ...
            <div className="p-8 sm:p-12 lg:p-16 space-y-8 text-left flex-grow">
                <div className="flex flex-wrap justify-between items-start gap-6">
                    ...
                    <div className="flex gap-4 shrink-0" onClick={(e) => e.stopPropagation()}>
                        {project.githubUrl && (
                ...
                {!isOpen && (
                    <div
                        className="w-full pt-4 text-xs sm:text-sm font-bold uppercase tracking-[0.5em] text-white/60 hover:text-primary-blue transition-all duration-300 text-center"
                    >
                        Explorar Proyecto [ + ]
                    </div>
                )}
